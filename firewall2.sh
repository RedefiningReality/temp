...
get_input_rule_num() {
  local iface=$1
  local ifnum
  # Only match simple ethN/bondN style: all-alpha prefix then all-digit suffix
  ifnum=$(echo "$iface" | grep -oP '^[a-z]+\K[0-9]+$')
  if [[ -n "$ifnum" ]]; then
    # eth0=10, eth1=20, ... eth9=100 (numeric interfaces, range 10-100)
    echo $(( (ifnum + 1) * 10 ))
  else
    # Non-numeric suffix: sum ASCII values of name chars mod 400 + 200
    # Gives a stable number in range 200-599, well above numeric iface range
    local sum=0
    local val
    for (( i=0; i<${#iface}; i++ )); do
      val=$(printf '%s' "${iface:$i:1}" | od -An -tu1 | tr -d ' ')
      sum=$(( sum + val ))
    done
    echo $(( (sum % 400) + 200 ))
  fi
